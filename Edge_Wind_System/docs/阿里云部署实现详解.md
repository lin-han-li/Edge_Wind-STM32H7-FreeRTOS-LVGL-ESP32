...
> 当前阿里云实例基线（2026-05-17 已核对）：代码目录为 `/var/www/edge_wind/Edge_Wind_System`，systemd 服务名为 `edge_wind.service`，Gunicorn 以 `--worker-class eventlet -w 1 --bind 127.0.0.1:5001 app:app` 运行，Nginx 将 `edge_wind` 站点反代到 `127.0.0.1:5001` 和 `127.0.0.1:5001/socket.io`。监控推送默认值为 `8Hz / 1024 waveform / 512 spectrum`。
...
> 不建议直接把 Flask/Gunicorn 的内部端口暴露公网；让 Nginx 做统一入口即可。

当前阿里云实例的内部端口是 5001。如果你暂时不做 Nginx，也可以临时放行该内部端口：
- TCP **5001**（临时测试用，不推荐长期）
...
EDGEWIND_MONITOR_EMIT_HZ=8

# 降采样点数（历史曲线/波形量大时可降低）
EDGEWIND_WAVEFORM_POINTS=1024
EDGEWIND_SPECTRUM_POINTS=512
...
```bash
sudo tee /etc/systemd/system/edge_wind.service > /dev/null <<'EOF'
[Unit]
...
sudo systemctl enable --now edge_wind
sudo systemctl status edge_wind --no-pager
```

查看日志：

```bash
sudo journalctl -u edge_wind -f
```
