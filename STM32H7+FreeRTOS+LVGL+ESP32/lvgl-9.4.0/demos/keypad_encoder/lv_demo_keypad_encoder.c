...
static void text_input_create(lv_obj_t *parent) {
  ...
  /* === 添加测试代码 === */
  /* 强制把键盘按键文字设为黑色，并指定字体 */
  static lv_style_t style_kb_test;
  lv_style_init(&style_kb_test);
  lv_style_set_text_color(&style_kb_test, lv_color_black());
  lv_style_set_text_font(&style_kb_test,
                         &lv_font_montserrat_14); // 确保用 14 号字体

  /* 应用到键盘的 ITEMS (按键) 部分 */
  lv_obj_add_style(kb, &style_kb_test, LV_PART_ITEMS);
  /* =================== */
