...
static volatile bool g_screen_loading = false;

static void screen_loaded_event_cb(lv_event_t * e)
{
    if (lv_event_get_code(e) == LV_EVENT_SCREEN_LOADED) {
        g_screen_loading = false;
        lv_indev_reset(NULL, NULL);
        lv_obj_t * target = lv_event_get_target(e);
        if (target) {
            lv_obj_remove_event_cb(target, screen_loaded_event_cb);
        }
    }
}

static void screen_load_timeout_cb(lv_timer_t * t)
{
    g_screen_loading = false;
    lv_indev_reset(NULL, NULL);
    lv_timer_delete(t);
...
#include "gg_external_data.h"
    ...
    if (*new_scr) {
        lv_obj_add_event_cb(*new_scr, screen_loaded_event_cb, LV_EVENT_SCREEN_LOADED, NULL);
        lv_timer_t * t = lv_timer_create(screen_load_timeout_cb, time + delay + 50U, NULL);
        if (t == NULL) {
            g_screen_loading = false;
        }
    } else {
